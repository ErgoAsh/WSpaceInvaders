...
public class SniperBoss extends Enemy {

	public SniperBoss(final Model copyFrom, final Vector3 position, final Vector3 target, final double health) {
		super(copyFrom, position, target, health);
	}
	
	@Override
	public void update(final float delta) {
		if (Math.random() < 0.075) { //TODO set to another color
			Main.getInstance().getStorage().addShot(new Shot(
					EModel.ENEMY, 
					transform.getTranslation(Utils.checker).cpy(), 
					Main.getInstance().getStorage().ship.transform.getTranslation(Utils.checker).cpy(), 
					false, 
					1.0D));
		}
		super.update(delta);
